...
4. Rewrite `competitor-analysis.md` next. Do not copy only the landing page's named competitors. Build a realistic competitor set from public research: direct startup/product competitors, larger incumbent tools, platform-native alternatives, and adjacent workflow substitutes that buyers would actually compare against. Prioritize main competitors by buyer relevance, adoption, category ownership, and market presence, not only similarity to the homepage copy. Include a short `## Verified competitor links` section near the top with exactly six official links, ordered strongest first. Use canonical product or company pages, not help-center/support URLs. Do not include obscure, tiny, or hobby/open-source tools in the top six unless they are clearly material to the buyer's decision. Each item must be a Markdown link using the official company or product website, for example `- [Competitor Name](https://example.com)`. Only include links you have verified; omit uncertain competitors instead of guessing domains.
